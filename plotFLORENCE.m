function [] = plotFLORENCE(dataset,study,subject,session,figNum1, figNum2)
% plotFLORENCE - provides plots of FLORENCE datasets to visually inspect 
%                FLORENCE datasets, with study, subject and session tags.

% INPUT VARIABLES:
    % dataset:  - Synchronised timetable of FLORENCE data
    % study:    - Study ID tag for recording (for plot titles)
    % subject:  - Subject tag for recording
    % session:  - Session tag for recording
...
figure(figNum1)
sgtitle([study '\_' subject '\_Session' session])
subplot(3,1,1)
